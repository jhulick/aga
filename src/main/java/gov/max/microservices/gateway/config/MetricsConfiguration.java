...
public class MetricsConfiguration extends MetricsConfigurerAdapter {
    ...
    public void init() {
        ...
        if (maxProperties.getMetrics().getJmx().isEnabled()) {
    ...
    public static class GraphiteRegistry {
        ...
        @PostConstruct
        private void init() {
            if (maxProperties.getMetrics().getGraphite().isEnabled()) {
                log.info("Initializing Metrics Graphite reporting");
                String graphiteHost = maxProperties.getMetrics().getGraphite().getHost();
                Integer graphitePort = maxProperties.getMetrics().getGraphite().getPort();
                String graphitePrefix = maxProperties.getMetrics().getGraphite().getPrefix();
    ...
    public static class SparkRegistry {
        ...
        @PostConstruct
        private void init() {
            if (maxProperties.getMetrics().getSpark().isEnabled()) {
                log.info("Initializing Metrics Spark reporting");
                String sparkHost = maxProperties.getMetrics().getSpark().getHost();
                Integer sparkPort = maxProperties.getMetrics().getSpark().getPort();
